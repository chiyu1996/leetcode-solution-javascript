...
var respace = function(dictionary, sentence) {
  const insert = (root, word) => {
    ...
    for (let i = word.length - 1; i >= 0; i--) {
      const now = word.charCodeAt(i) - 'a'.charCodeAt(0);
  ...
  const root = new point();
  const length = sentence.length;
  for (const word of dictionary) {
    insert(root, word);
  }

  const dp = new Array(length + 1).fill(Infinity);
  dp[0] = 0;
  ...
  for (let i = 1; i <= sentence.length; i++) {
    ...
    let cur = root;
    for (let j = i; j >= 1; j--) {
      const now = sentence[j - 1].charCodeAt(0) - 'a'.charCodeAt(0);
      if (cur.next[now] === undefined) {
        break;
