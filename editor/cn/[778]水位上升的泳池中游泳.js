...
var swimInWater = function (grid) {
    ...
    const dfs = (T) => {
        ...
        while (stack.length) {
            let t = stack.pop();
            let row = Math.floor(t / n);
            let col = t % n;
            if (row === n - 1 && col === n - 1) {
                return true;
            }
            for (let [offsetR, offsetC] of dirs) {
                let newRow = row + offsetR;
                let newCol = col + offsetC;
                let newT = newRow * n + newCol;
                if (newRow >= 0 && newRow < n && newCol >= 0 && newCol < n && !set.has(newT) && grid[newRow][newCol] <= T) {
                    stack.push(newT);
                    set.add(newT);
                }
            }
