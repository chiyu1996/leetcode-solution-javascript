...
var findSubstring = function(s, words) {
  ...
  const word_num = words.length;
  const one_word = words[0].length;
  const n = s.length;
  ...
  for (let i = 0; i < one_word; i++) {
    let left = i;
    let right = i;
    const cur_map = Counter();
    let cnt = 0;
    while (right + one_word <= n) {
      const w = s.slice(right, right + one_word);
      right += one_word;
      if (words.get(w)) {
        cnt += 1;
        if (cur_map.get(w)) {
        ...
        while (cur_map.get(w) > words.get(w)) {
          const left_w = s.slice(left, left + one_word);
          left += one_word;
